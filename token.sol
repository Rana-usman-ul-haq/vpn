...
contract VirtualPinNetwork is Context, IERC20, Ownable {
    ...
    function setFeePercent(uint256 liquidityFee, uint256 marketingFee, uint256 charityFee, uint256 taxFee, uint256 burnFee) external onlyOwner() {
        require(liquidityFee.add(marketingFee).add(charityFee).add(taxFee).add(burnFee) <= 25, "tax too high");
